...
class MyTestClass(TestCase):
    ...
    def test_demand_overrides(self):
        ...
        demand.apply_overrides({"constant_demand": 20,
                                "pollutant_load": {"phosphate": 0.5}
                                })
        self.assertEqual(demand.constant_demand, 20)
        self.assertDictEqual(demand.pollutant_load, {"phosphate": 0.5, "temperature": 12})
    
    def test_residentialdemand_overrides(self):
        demand = ResidentialDemand(name="", gardening_efficiency=0.4, pollutant_load={"phosphate": 0.1, "temperature": 12})
        demand.apply_overrides({"gardening_efficiency": 0.5,
                                "population": 153.2,
                                "per_capita": 32.4,
