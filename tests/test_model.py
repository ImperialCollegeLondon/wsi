...
class MyTestClass(TestCase):
    ...
    def test_add_nodes(self):
        ...
        surface1 = {'type_' : 'ImperviousSurface',
                    'surface' : 'urban',
                    'area' : 10,
                    'pollutant_load' : {'phosphate' : 1e-7}}

        surface2 = {'type_' : 'PerviousSurface',
                    'surface' : 'rural',
                    'area' : 100,
                    'depth' : 0.5,
                    'pollutant_load' : {'phosphate' : 1e-7}}
    ...
    def test_add_arcs(self):
        ...
        arc = {'in_port' : 'my_land',
                    ...
                    'capacity' : 2,
                    'preference' : 2.5}
        arc2 = {'in_port' : 'my_node',
                    'out_port' : 'my_waste',
                    'name' : 'my_arc2',
                    'type_' : 'Arc'
                    }
        my_model = Model()
        my_model.add_nodes([node, 
                                            waste,
                                            land])
        my_model.add_arcs([arc,
                                            arc2])
    ...
    def test_add_ins_nodes(self):
        ...
        land = Land(**{
                ...
                'name' : 'my_land'})
        my_model = Model()
        my_model.add_instantiated_nodes([sewer, 
                    land])
    ...
    def test_add_ins_arcs(self):
        ...
        arc = Arc(**{'in_port' : land,
                    'out_port' : node,
                    'name' : 'my_arc',
                    'capacity' : 2,
                    'preference' : 2.5})
        arc2 = Arc(**{'in_port' : node,
                    'out_port' : land,
                    'name' : 'my_arc2',
                    })
        my_model = Model()
        my_model.add_instantiated_nodes([node, 
                                            waste,
                                            land])
        my_model.add_instantiated_arcs([arc,
                                            arc2])
        self.assertEqual(2, my_model.arcs['my_arc'].capacity)
        self.assertEqual(2.5, my_model.arcs['my_arc'].preference)
    ...
    def test_run(self):
        ...
        dates = [date]
        land_inputs = {('temperature', date) : 10,
        ...
        urban_drainage = {'type_' : 'Arc',
        'in_port' : 'my_land',
        'out_port' : 'my_sewer',
        'name' : 'urban_drainage'}

        percolation = {'type_' : 'Arc',
                'in_port' : 'my_land',
        ...
        my_model.add_arcs([urban_drainage, 
                   percolation, 
                   runoff, 
                   storm_outflow, 
                   baseflow, 
        ...
        self.assertEqual(0.03, my_model.nodes['my_land'].get_surface('urban').storage['volume'])
