...
class TestRealModel(TestCase):
    ...
    def test_performance_comparison(self):
        ...
        model_path = r"C:\Users\bdobson\OneDrive - Imperial College London\temp\bwick\v2\model"
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            def load_unified_model():
                ...
                model = Model()
                model.load_unified_data(os.path.join(parquet_dir, 'unified_data.parquet'))
                return model
            ...
            save_original_time = timeit.timeit(
                lambda: original_model.save(csv_dir, compress=False),
                number=1
            )
            print(f"Original save time: {save_original_time:.4f} seconds")
            
            # Load original model
            load_original_time = timeit.timeit(
                lambda: load_original_model(),
                number=1
            )
            ...
            run_original_time = timeit.timeit(
                lambda: run_model(original_model),
                number=1
            )
            ...
            save_unified_time = timeit.timeit(
                lambda: original_model.save_unified_data(parquet_dir),
                number=1
            )
            ...
            run_unified_time = timeit.timeit(
                lambda: run_model(unified_model),
                number=1
            )
            ...
            save_improvement = ((save_original_time - save_unified_time) / save_original_time) * 100
            load_improvement = ((load_original_time - load_unified_time) / load_original_time) * 100
            run_improvement = ((run_original_time - run_unified_time) / run_original_time) * 100
            ...
            total_improvement = ((total_original - total_unified) / total_original) * 100
            ...
            self.assertEqual(len(original_results[0]), len(unified_results[0]), "Flow results length mismatch")
            self.assertEqual(len(original_results[1]), len(unified_results[1]), "Tank results length mismatch")
            self.assertEqual(len(original_results[3]), len(unified_results[3]), "Surface results length mismatch")
