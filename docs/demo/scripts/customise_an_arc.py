...
# play around. On the GitHub it is in docs/demo/scripts/customise_an_arc.py
#
...
# %%
apply_variable_capacity(customised_model.arcs['abstraction_to_farmoor'], 
                        levels, 
                        capacity_multiplier)
...
plot_flows1 = pd.concat([baseline_flows.groupby('arc').get_group('farmoor_to_mixer').set_index('time').flow.rename('Baseline'),
                        flows_var_cap.groupby('arc').get_group('farmoor_to_mixer').set_index('time').flow.rename('Customised')],
                       axis=1)
...
plot_flows2 = pd.concat([baseline_flows.groupby('arc').get_group('abstraction_to_farmoor').set_index('time').flow.rename('Baseline'),
                        flows_var_cap.groupby('arc').get_group('abstraction_to_farmoor').set_index('time').flow.rename('Customised')],
                       axis=1)
...
plot_tanks = pd.concat([baseline_tanks.groupby('node').get_group('farmoor').set_index('time').storage.rename('Baseline'),
                        tanks_var_cap.groupby('node').get_group('farmoor').set_index('time').storage.rename('Customised')],
                       axis=1)
