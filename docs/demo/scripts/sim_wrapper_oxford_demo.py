# -*- coding: utf-8 -*-
"""
Created on Wed Feb 8 09:09:52 2023
...
# if we want to submit it to HPC then the next line is needed
...
def sim_wrapper(par):
    '''
    A simulation wrapper for the Oxford demo, which now accepts parameters as input
    
    Parameters
    ----------
    par : (dictionary)
        A dictionary contains all potential parameters as input

    Returns
    -------
    flow results of arcs

    '''
    ### parameter setting
    # FWTW parameters
    service_reservoir_storage_capacity = par['service_reservoir_storage_capacity']
    service_reservoir_storage_area = par['service_reservoir_storage_area']
    service_treatment_throughput_capacity = par['service_treatment_throughput_capacity']
    # Land parameters
    # pervious surface
    total_porosity = par['total_porosity']
    field_capacity = par['field_capacity']
    wilting_point = par['wilting_point']
    infiltration_capacity = par['infiltration_capacity']
    surface_coefficient = par['surface_coefficient']
    percolation_coefficient = par['percolation_coefficient']
    et0_coefficient = par['et0_coefficient']
    ihacres_p = par['ihacres_p']
    subsurface_residence_time = par['subsurface_residence_time']
    percolation_residence_time = par['percolation_residence_time']
    surface_residence_time = par['surface_residence_time']
    # impervious surface
    pore_depth = par['pore_depth']
    # WWTW parameters
    stormwater_storage_capacity = par['stormwater_storage_capacity']
    stormwater_storage_area = par['stormwater_storage_area']
    stormwater_treatment_throughput_capacity = par['stormwater_treatment_throughput_capacity']
    # sewer capacity
    sewer_capacity = par['sewer_capacity']
    # gw capacity
    gw_capacity = par['gw_capacity']
    gw_residence_time = par['gw_residence_time']
    ...
    oxford_fwtw = FWTW(service_reservoir_storage_capacity = service_reservoir_storage_capacity,
                      service_reservoir_storage_area = service_reservoir_storage_area,
                      treatment_throughput_capacity = service_treatment_throughput_capacity,
                      name = 'oxford_fwtw')
    ...
    surface = [{'type_' : 'PerviousSurface',
              ...
              'pollutant_load' : pollutant_deposition,
               ...
               'surface_coefficient' : surface_coefficient,
    ...
    oxford_land = Land(surfaces = surface,
                       ...
                       surface_residence_time = surface_residence_time,
                       subsurface_residence_time = subsurface_residence_time,
                       percolation_residence_time = percolation_residence_time
                       )
    ...
    oxford_wwtw = WWTW(stormwater_storage_capacity = stormwater_storage_capacity,
                      stormwater_storage_area = stormwater_storage_area,
                      treatment_throughput_capacity = stormwater_treatment_throughput_capacity,
                      name = 'oxford_wwtw')
    ...
    # %%
    combined_sewer = Sewer(capacity = sewer_capacity,
                           pipe_timearea = {0 : 0.8,
                                            1 : 0.15,
                                            2 : 0.05
                                            },
    ...
    gw = Groundwater(capacity = gw_capacity,
                     area = 3.2e8,
                     name = 'gw',
    ...
    return flows
#############################
### test the sim wrapper ###
#############################
# initialise parameter input
par = {# FWTW parameters
       'service_reservoir_storage_capacity' : 1e5,
       'service_reservoir_storage_area' : 2e4,
       'service_treatment_throughput_capacity' : 4.5e4,
       # Land parameters
       # pervious surface
       'total_porosity' : 0.4,
       'field_capacity' : 0.3,
       ...
       'stormwater_treatment_throughput_capacity' : 5e4,
       # sewer capacity
       'sewer_capacity' : 4e6,
       # gw capacity
       'gw_capacity' : 3.2e9,
       'gw_residence_time' : 500
       }
# simulate
flows = sim_wrapper(par)
flows = pd.DataFrame(flows)

print(flows.sample(10))
# read observed data
dates = pd.to_datetime(flows.loc[flows.arc == flows.arc.unique()[0], 'time']).to_list()
...
### optimise wrapper ###
########################
def optimise_wrapper(par):
    '''
    A optimisation wrapper for calibrating the Oxford demo, which now accepts parameters as input
    ...
    Returns
    -------
    objective functions and constratints for optimisation
    ...
    for pol in set(val_pollutants):
        if pol != 'temperature':
            flows[pol] /= flows.flow
    ### calculate objective functions - currently water quality only
    objs = [] # rmse * 4 pollutants
    validation_arcs = ['mixer_to_waste']
    for arc in validation_arcs:
        for pol in val_pollutants:
            sim = flows_plot.loc[flows_plot.arc == plot_arc, [pol,'time']].set_index('time')
            obs = mixer_val_df[pol]
            common_idx = sim.index.intersection(obs.index)
            sim = sim.loc[common_idx, pol]
            obs = obs.loc[common_idx]
            
            rmse = (sum((sim - obs)**2)/len(sim))**0.5
            objs.append(rmse)
    ### constraints
    constraints = []
    constraints.append(par['field_capacity'] - par['total_porosity'])
    constraints.append(par['wilting_point'] - par['field_capacity'])
    constraints.append(par['surface_residence_time'] - par['subsurface_residence_time'])
    constraints.append(par['subsurface_residence_time'] - par['gw_residence_time'])
...
problem = Problem(21, # no. of variables to be optimised
                  4)  # no. of objective functions
# formulate variable constraints
constraints = {# FWTW parameters
               'service_reservoir_storage_capacity' : [1e-9, 1e9],
               'service_reservoir_storage_area' : [1e-9, 1e9],
               'service_treatment_throughput_capacity' : [1e-9, 1e9],
               # Land parameters
               # pervious surface
               'total_porosity' : [1e-9, 0.5],
               'field_capacity' : [1e-9, 0.5],
               'wilting_point' : [1e-9, 0.5],
               'infiltration_capacity' : [1e-9, 0.7],
               'surface_coefficient' : [1e-9, 0.999],
               'percolation_coefficient' : [1e-9, 0.999],
               'et0_coefficient' : [1e-9, 1.5],
               'ihacres_p' : [1e-9, 1e9],
               'subsurface_residence_time' : [1e-9, 100],
               'percolation_residence_time' : [1e-9, 1000],
               'surface_residence_time' : [1e-9, 10],
               # impervious surface
               'pore_depth' : [1e-9, 1],
               # WWTW parameters
               'stormwater_storage_capacity' : [1e-9, 1e9],
               'stormwater_storage_area' : [1e-9, 1e9],
               'stormwater_treatment_throughput_capacity' : [1e-9, 1e9],
               # sewer capacity
               'sewer_capacity' : [1e-9, 1e9],
               # gw capacity
               'gw_capacity' : [1e-9, 1e9],
               'gw_residence_time' : [1e-9, 1000]
               }
types = []
for key, constraint in constrants.items():
    types.append(Real(constraint[0], constraint[1]))

problem.types[:] = types
problem.function = optimise_wrapper
problem.directions[:] = Problem.MINIMIZE
problem.constraints[:] = '<=0'

with ProcessPoolEvaluator(64) as evaluator:
    population_size = 100
    no_of_generations= 10000
    algorithm = NSGAII(problem, 
                       population_size = population_size,
                       evaluator = evaluator)
    algorithm.run(no_of_generations)

# save optimised results
import pickle
output_address = os.path.join("Opt"+str(population_size)+"population_size_"+str(no_of_generations)+"_generations.pkl")
