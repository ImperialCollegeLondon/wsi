...
for axs, variable in zip(axs_.T, ['flow','phosphate']):
    ...
    input_data.pivot(index = 'date',
                     columns = 'variable',
                     values = 'value')[['precipitation']].plot(ax=axs[0], xlabel='',xticks=[],sharex=True)
    
    flows_plot[['runoff','urban_drainage','percolation']].plot(ax=axs[1], xlabel ='',xticks=[],sharex=True)
