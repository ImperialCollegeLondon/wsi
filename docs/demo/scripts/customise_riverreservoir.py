...
# %%
# Import packages
from wsimod.nodes.storage import RiverReservoir, Groundwater
from wsimod.nodes.waste import Waste
from wsimod.arcs.arcs import Arc
from wsimod.orchestration.model import to_datetime, Model
...
# %%
# Create nodes
my_reservoir = RiverReservoir(name = 'my_reservoir',
                              capacity = 10,
                              area = 5,
                              initial_storage = 6,
                              environmental_flow = 1)

my_groundwater = Groundwater(name = 'my_groundwater',
                             capacity = 100,
                             area = 10,
                             initial_storage = 50)
...
# %%
# Create arcs
infiltration_arc = Arc(name = 'infiltration',
                       in_port = my_reservoir,
                       out_port = my_groundwater)

environmental_arc = Arc(name = 'environmental',
                        in_port = my_reservoir,
                        out_port = my_outlet)

baseflow_arc = Arc(name = 'baseflow',
...
my_model.add_instantiated_nodes([my_reservoir,
                                 my_groundwater,
                                 my_outlet])

my_model.add_instantiated_arcs([infiltration_arc,
                                environmental_arc,
                                baseflow_arc])
...
# %%
# Create climate data
climate_data = {('precipitation',date) : 0.01,
                 ('et0',date) : 0.005
                 }
...
# Run decorator
my_reservoir.make_abstractions = wrapper(my_reservoir,
                                         my_reservoir.make_abstractions)
