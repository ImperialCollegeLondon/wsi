...
data_folder= os.path.join("D:\\", "Huawei", "PhD", "Year4", "cherwell_demo")
...
land.subsurface_runoff.storage['volume'] = 15e7
land.subsurface_runoff.outflow['volume'] = 4764960
...
my_model.add_instantiated_nodes([land,discharge_point,river,outlet,gw])
my_model.add_instantiated_arcs([arc1,arc2,arc3,arc4,arc5])
...
x = flows.groupby('arc').get_group('land-river').set_index('time').index
y = flows.groupby('arc').get_group('land-river').set_index('time').flow / constants.M3_S_TO_M3_DT
#x = surfaces.groupby('surface').get_group('my_growing_surface0').set_index('date').index
y = surfaces.groupby('surface').get_group('my_growing_surface9').surface_flow
#y = surfaces.groupby('surface').get_group('my_growing_surface7').nutrient_pool / land.surfaces[7].area * 1e6
axs[0].plot(x, y)
#axs[0].set_xlim([x[-365*20], x[-1]])
#axs[0].set_ylim([0,2e6])
x = flows.groupby('arc').get_group('land-river').set_index('time').index
y = flows.groupby('arc').get_group('land-river').set_index('time').nitrate
y = surfaces.groupby('surface').get_group('my_growing_surface9').subsurface_flow
# y = (surfaces.groupby('surface').get_group('my_growing_surface7').nitrate + \
#       surfaces.groupby('surface').get_group('my_growing_surface7').nitrite + \
#       surfaces.groupby('surface').get_group('my_growing_surface7').ammonia) / land.surfaces[7].area * 1e6
axs[1].plot(x, y)
axs[1].set_xlim([x[-365*20], x[-1]])
f, axs = plt.subplots()
x = flows.groupby('arc').get_group('land-river').set_index('time').index
y = flows.groupby('arc').get_group('land-river').set_index('time').phosphate
#y = surfaces.groupby('surface').get_group('my_growing_surface2').crop_uptake_N  / land.surfaces[2].area * 1e6#surface_flow
y = surfaces.groupby('surface').get_group('my_growing_surface5').subsurface_flow / surfaces.groupby('surface').get_group('my_growing_surface9').surface_flow *1e3
y = flows.groupby('arc').get_group('land-river').set_index('time').nitrate
axs.plot(x, y)
axs.set_xlim([x[-365*20], x[-1]])
#axs.set_ylim([0,20])
f, axs = plt.subplots()
x = flows.groupby('arc').get_group('land-river').set_index('time').index
y = flows.groupby('arc').get_group('land-river').set_index('time').nitrate + \
    flows.groupby('arc').get_group('land-river').set_index('time').nitrite + \
    flows.groupby('arc').get_group('land-river').set_index('time').ammonia
y = surfaces.groupby('surface').get_group('my_growing_surface7').crop_factor
axs.plot(x, y)
axs.set_xlim([x[-365*2], x[-1]])


f, axs = plt.subplots()
for i in range(0, len(land.surfaces)-1):
    y = surfaces.groupby('surface').get_group('my_growing_surface'+str(i)).subsurface_r
    axs.plot(x, y, label = i)
    axs.set_xlim([x[-365*5], x[-1]])
axs.legend()

# f, axs = plt.subplots(4,1)
# x = flows.groupby('arc').get_group('wwtw-river').set_index('time').index
# y = flows.groupby('arc').get_group('wwtw-river').set_index('time').flow / constants.M3_S_TO_M3_DT
# axs[0].plot(x, y)
# x = flows.groupby('arc').get_group('wwtw-river').set_index('time').index
# y = flows.groupby('arc').get_group('wwtw-river').set_index('time').phosphate
# axs[1].plot(x, y)
# x = flows.groupby('arc').get_group('wwtw-river').set_index('time').index
# y = flows.groupby('arc').get_group('wwtw-river').set_index('time').nitrate + \
#     flows.groupby('arc').get_group('wwtw-river').set_index('time').nitrite + \
#     flows.groupby('arc').get_group('wwtw-river').set_index('time').ammonia
# axs[2].plot(x, y)
# x = flows.groupby('arc').get_group('wwtw-river').set_index('time').index
# y = flows.groupby('arc').get_group('wwtw-river').set_index('time').solids
# axs[3].plot(x, y)
