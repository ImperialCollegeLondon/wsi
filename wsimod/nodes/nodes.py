...
class Node(WSIObj):
    ...
    def pull_distributed(self, vqip, of_type = None, tag = 'default'):
        ...
        Iterate until request is met or maximum iterations are hit. 
...
class Tank(WSIObj):
    ...
    def __init__(self,
        ...
        if 'initial_storage' in dir(self):
            if isinstance(self.initial_storage, dict):
                ...
                self.storage_ = self.copy_vqip(self.initial_storage) #Lagged storage for mass balance
            else:
                #Assume number describes initial stroage
                self.storage = self.v_change_vqip(self.empty_vqip(), self.initial_storage)
                self.storage_ = self.v_change_vqip(self.empty_vqip(), self.initial_storage) #Lagged storage for mass balance
        else:
    ...
    def pull_storage(self, vqip):
        """Pull water from tank, updating the storage VQIP. Pollutants are 
        removed from tank in proportion to 'volume' in vqip (pollutant values 
        in vqip are ignored)
    ...
    def pull_pollutants(self, vqip):
        """Pull water from tank, updating the storage VQIP. Pollutants are 
        removed from tank in according to their values in vqip
    ...
    def evaporate(self, evap):
        """Wrapper for v_distill_vqip to apply a volumetric subtraction from 
        tank storage. Volume removed from storage and no change in pollutant 
        values.
...
class QueueTank(Tank):
    def __init__(self, 
        ...
        storage is available for use. The storage that has completed the queue 
        is under the 'active_storage' property.
    ...
    def pull_storage_exact(self, vqip):
        """Pull storage from the QueueTank, only water in active_storage is 
        available. Pollutants are removed from tank in according to their values in 
        vqip
